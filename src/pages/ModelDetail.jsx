    ...
    return (
      <CossPage>
        <div className="flex min-h-[50vh] items-center justify-center">
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-page-divider border-t-brand-500" />
        </div>
      </CossPage>
    );
  }
    ...
    <CossPage>
      <section className="coss-page-header">
        <CossContainer className="py-8 sm:py-10 lg:py-12">
          <Link to="/models" className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-page-secondary hover:text-page">
            <ArrowLeft size={16} />
            Models
          </Link>
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="min-w-0">
              ...
              <h1 className="text-3xl font-semibold tracking-normal text-page sm:text-4xl">{modelName}</h1>
              <p className="mt-4 max-w-3xl text-sm leading-6 text-page-secondary">{getModelSummary(model)}</p>
              <div className="mt-4 flex max-w-3xl flex-wrap items-center gap-2 rounded-lg border border-page-divider bg-page-surface/50 px-3 py-2 shadow-sm">
                <code className="min-w-0 flex-1 truncate font-mono text-sm text-page-secondary">{id}</code>
            ...
            <CossCardFrame as="aside" className="p-5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-page">Pricing and API</h2>
              </div>
        ...
        <div className="space-y-6">
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <InfoPanel title="About" icon={Layers3}>
              <p className="text-sm leading-6 text-page-secondary">{getModelIntro(model)}</p>
            </InfoPanel>
              ...
              <ul className="space-y-2 text-sm leading-6 text-page-secondary">
                {useCases.map((item) => (
                  <li key={item} className="flex gap-2">
                    <CheckCircle2 size={16} className="mt-1 flex-shrink-0 text-page-success" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {capabilities.map((item) => (
                  <CossMutedCard key={item.label} className="p-4">
                    <p className="text-xs font-medium text-page-muted">{item.label}</p>
                    <p className="mt-2 break-words text-sm font-semibold text-page">{item.value}</p>
                  </CossMutedCard>
                ))}
              </div>
            </InfoPanel>
          ...
          <CossCardFrame className="p-5 sm:p-6">
            <div className="flex items-center gap-2">
              <CossIconTile icon={Server} />
              <h2 className="text-xl font-semibold text-page">API usage</h2>
            </div>
  ...
  return (
    <CossCard className="p-5 sm:p-6" as="section">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-xl font-semibold text-page">{title}</h2>
      </div>
      {children}
    </CossCard>
  );
...
function MetaRow({ label, value, copy = false }) {
  return (
    <div>
      <dt className="text-xs text-page-muted">{label}</dt>
      <dd className="mt-1 flex items-center gap-2 font-mono text-sm text-page-secondary">
