...
export default function Account() {
  ...
  const loadInvoice = useCallback(async () => {
    ...
    try {
      ...
      // The shared API interceptor handles user-facing errors.
    } finally {
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      <div className="mb-8">
        <h1 className="mb-1 text-2xl font-heading font-bold text-page">{t('account.title')}</h1>
      ...
      {site?.enable_invoice && (
        <section id="invoice" className="glass mt-6 scroll-mt-24 rounded-2xl p-6">
          <div className="mb-5 flex items-center gap-2">
          ...
          <form onSubmit={handleInvoiceSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <TextField label={t('invoice.titleField')} value={invoiceInfo.title} onChange={(value) => updateInvoiceInfo('title', value)} />
            <TextField label={t('invoice.taxId')} value={invoiceInfo.tax_id} onChange={(value) => updateInvoiceInfo('tax_id', value)} />
            <TextField label={t('invoice.email')} type="email" value={invoiceInfo.email} onChange={(value) => updateInvoiceInfo('email', value)} />
            <TextField label={t('invoice.country')} value={invoiceInfo.country} onChange={(value) => updateInvoiceInfo('country', value)} />
            <TextField label={t('invoice.contactName')} value={invoiceInfo.contact_name} onChange={(value) => updateInvoiceInfo('contact_name', value)} />
            <TextField label={t('invoice.phone')} value={invoiceInfo.phone} onChange={(value) => updateInvoiceInfo('phone', value)} />
            <label className="block md:col-span-2">
              <span className="mb-1.5 block text-sm font-medium text-page-label">{t('invoice.address')}</span>
              <textarea className="input min-h-[88px]" value={invoiceInfo.address} onChange={(event) => updateInvoiceInfo('address', event.target.value)} />
            </label>
            <label className="block md:col-span-2">
              <span className="mb-1.5 block text-sm font-medium text-page-label">{t('invoice.extra')}</span>
              <textarea className="input min-h-[88px]" value={invoiceInfo.extra} onChange={(event) => updateInvoiceInfo('extra', event.target.value)} />
            </label>
